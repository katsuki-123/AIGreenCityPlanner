...
def call_gemini(prompt):
    ...
    try:
        ...
        st.warning("Install google-genai to enable Gemini responses.")
    except Exception as exc:
        st.warning(f"Gemini request failed: {exc}")

    return None


def ward_context(ward_row):
    optimal_target = 95.0
    current_green = float(ward_row["After %"])
    gap_to_optimal = max(0, optimal_target - current_green)
    if current_green >= optimal_target:
        optimal_status = "Already at or above the dashboard optimal greenery target."
    elif gap_to_optimal <= 5:
        optimal_status = "Very close to the dashboard optimal greenery target."
    elif gap_to_optimal <= 15:
        optimal_status = "Moderately below the dashboard optimal greenery target."
    else:
        optimal_status = "Far below the dashboard optimal greenery target."

    return "\n".join(
        [
            f"Ward: {ward_row['Ward']}",
            f"Green coverage before: {ward_row['Before %']}%",
            f"Green coverage after: {ward_row['After %']}%",
            f"Increase: {ward_row['Increase %']}%",
            f"Dashboard optimal greenery target: {optimal_target}%",
            f"Gap to dashboard optimal target: {round(gap_to_optimal, 2)} percentage points",
            f"Optimal greenery status: {optimal_status}",
            f"Baseline AQI: {ward_row['Baseline AQI']}",
            f"Final AQI: {ward_row['Final AQI']}",
            f"AQI improvement: {ward_row['AQI Improvement']}",
...
def generate_report_insights(ward_row):
    prompt = f"""
Write a concise ward planning report section using only the facts below.
Format: short executive summary, key interpretation, three practical recommendations.

{ward_context(ward_row)}
"""
    return call_gemini(prompt)


def get_ward_chat_response(ward_row, user_question):
    question = user_question.strip() or "Summarize this ward and explain the next best action."
    prompt = f"""
You are a Mumbai green planning chatbot. Give a direct useful answer first, then support it with numbers.
Do not start with "I'm sorry" unless the question is unrelated to the dashboard.

Rules:
- Treat the dashboard optimal greenery target as 95%.
- 95% or higher means already at/near optimal in this dashboard model.
- For "how long", say there is no calendar schedule, then give a planning estimate:
  already optimal: no wait needed; under 5-point gap: short-term infilling; 5-15 points: medium-term program; over 15 points: long-term intervention.
- Do not invent exact months, budgets, or plantation counts.
...
"""
    ...
    return "Gemini is not available right now. Please check your API key in .streamlit/secrets.toml and try again."


# ===============================
# PDF FUNCTION
# ===============================
def generate_pdf(ward_info, ai_insights=None):
    ...
    if ai_insights:
        ...
        elements.append(Paragraph("Gemini AI Planning Insights", styles["Heading2"]))
        for paragraph in ai_insights.split("\n"):
            if paragraph.strip():
                elements.append(Paragraph(html.escape(paragraph.strip()), styles["BodyText"]))
...
if st.session_state.active_page == "Dashboard":
    ...
    with tab4:
        if st.button("Generate PDF Report"):
            with st.spinner("Generating Gemini report insights..."):
                ai_insights = generate_report_insights(ward_data)
                pdf_path = generate_pdf(ward_data, ai_insights)
    ...
    with tab5:
        ...
        with st.container(border=True):
            ...
            if st.session_state.get("ward_chat_last_ward") != chat_ward:
                st.session_state["ward_chat_response"] = ""
                st.session_state["ward_chat_last_ward"] = chat_ward

            safe_ward_overview = html.escape(get_ward_chat_summary(chat_row)).replace(
                "\n",
                "<br>",
            )
            ...
            chat_question = st.text_area(
                "Ask Gemini about this ward",
                placeholder="Example: What should planners prioritize here?",
                key="ward_chat_question",
            )

            if st.button("Ask chatbot"):
                with st.spinner("Gemini is answering..."):
                    st.session_state["ward_chat_response"] = get_ward_chat_response(
                        chat_row,
                        chat_question,
                    )

            if st.session_state.get("ward_chat_response"):
                safe_chat_response = html.escape(st.session_state["ward_chat_response"]).replace(
                    "\n",
                    "<br>",
                )
